...
//n.b j+l*p^k è sempre diverso da t+h*p^k per ogni l ed h interi,quindi non bisogna memorizzare quali elementi dell'array sono stati divisi per p
    ...
    while(indexv<=double_M){//all'inizio j_temp=0*p+j1t,poi diventa k*p+j1t(a salti di p)
        thread_data.numbers[indexv].sum_log+=log_p;
        indexv+=p;
    }
    j_temp2=-p+j1;//all'inizio j_temp=-p+j1t,poi diventa -k*p+j1t(a salti di p negativi)
    indexv=j_temp2+M;
    while(indexv>=0){//all'inizio j_temp=0*p+j1t,poi diventa k*p+j1t(a salti di p)
        thread_data.numbers[indexv].sum_log+=log_p;
        indexv-=p;
    }
    if(j==1){
...
char divide_all_by_2_log(long M,struct thread_data thread_data){//divide gli elementi dell'array di numeri per 2
    ...
    const int log_p=r.log_prime[1];
    for(;i<2*M+1;i=i+2){
        //thread_data.numbers[i].last_index_f_base=1;//riga comm
        //thread_data.numbers[i].first_index_f_base=1;//riga comm
        thread_data.numbers[i].sum_log=log_p;
    }
